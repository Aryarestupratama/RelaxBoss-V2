...
<div x-show="showDeleteModal" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
    <div class="flex items-center justify-center min-h-screen px-4">
        <div x-show="showDeleteModal" x-transition class="fixed inset-0 transition-opacity" @click="showDeleteModal = false">
        ...
        <div x-show="showDeleteModal" x-transition class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
            <form :action="`/admin/quizzes/${deleteQuiz.id}`" method="POST">
                @csrf
                @method('DELETE')
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-lg leading-6 font-medium text-gray-900">Hapus Kuis</h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    Apakah Anda yakin ingin menghapus kuis <strong x-text="deleteQuiz.name"></strong>? Semua pertanyaan, jawaban, dan hasil terkait akan dihapus secara permanen.
                                </p>
                            </div>
                        </div>
                    </div>
